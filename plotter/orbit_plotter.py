...
planet.active_t_coords = np.zeros((planet.points.shape[0], 2))

planet.active_t_coords[:, 0] = 0.5 + np.arctan2(-planet.points[:, 0], planet.points[:, 1])/(2 * np.pi)
planet.active_t_coords[:, 1] = 0.5 + np.arcsin(planet.points[:, 2]) / np.pi
...
def orbitPlot(epoch, points, daynight=False):
	...
	# ----- SCENE PROPERTIES -----
	plotter.add_text(str(epoch), font='courier', font_size=12, position='lower_right')
